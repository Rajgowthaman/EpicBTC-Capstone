...
import time
import requests
# from sklearn.preprocessing import MinMaxScaler # Commented out scaling import
...
def scale_features(X):
    ...
    # return X_scaled
    return X.astype('float32') # Return original data without scaling
...
def rescale_prediction(y_scaled):
    ...
    # return y_rescaled
    y_rescaled = np.zeros(3)
    y_rescaled[0] = y_scaled[0]  # Close (no rescale)
    y_rescaled[1] = y_scaled[1]  # Volatility (no rescale)
    y_rescaled[2] = torch.sigmoid(torch.tensor(y_scaled[2])).item() # Direction probability
    return y_rescaled
...
if X_raw.shape[0] < 60:
    ...
    with torch.no_grad():
        ...
        close = prediction[0] #- 75673 #temporary cap  # Adjusted to not be scaled
        vol = abs(prediction[1]) # Adjusted to not be scaled
